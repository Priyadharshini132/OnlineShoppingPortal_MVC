...
public class SearchService_Model implements java.io.Serializable {
	...
	public ArrayList<BasicSearch_Controller> basicSearch(String word)
			throws SQLException// this one is called
	{
		...
		try {
			...
			for (int i = 0; i < tokens.length; i++) {
				...
				que3 = "SELECT SH.SHID, CAT.img, SH.PRICE,SH.SHOE_NAME, SH.DESCP FROM SHOE_DET SH, CAT_ATTR CAT "
						+ "WHERE SH.SHID= CAT.shid AND (SH.DESCP LIKE '%"
						+ tokens[i]
						+ "%') GROUP BY(SH.SHOE_NAME) order by SH.SHID;";
				...
				rs1.beforeFirst();
				System.out
						.println("Number of records retrieved from database: ");
			...
			try {
				...
				for (int i = 0; i < rows; i++) {
					BasicSearch_Controller newResult = makebasicsearch(jsonResults
							.getJSONObject(i));
					bs1.add(newResult);
				}
	...
	public static BasicSearch_Controller makebasicsearch(
