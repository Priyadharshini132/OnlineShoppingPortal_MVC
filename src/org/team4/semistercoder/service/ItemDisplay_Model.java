...
public class ItemDisplay_Model {
	...
	public static ItemDescription_Controller makebasicsearch1(
	...
	public ArrayList<ItemDescription_Controller> itemdescription(int shid)
			throws SQLException {
		...
		try {
			...
			conn10 = (Connection) DriverManager.getConnection(url, username,
					password);
			...
			que10 = "SELECT distinct SH.SHID, CA.img, SH.PRICE,SH.SHOE_NAME,SH.DESCP, B.B_NAME, "
					...
					+ "WHERE (SH.SHID="
					+ id
					+ ") AND SH.SHID= CA.shid AND SH.SHID=CM.shid AND CM.bid=B.BID "
			...
			while (rs10.next()) {
				...
				System.out
						.println("Number of columns retrieved from database: :");
				System.out.println(totalCol);
				JSONObject obj = new JSONObject();
				/**/
				for (int i = 0; i < totalCol; i++) {
					obj.put(rs10.getMetaData().getColumnLabel(i + 1)
							.toLowerCase(), rs10.getObject(i + 1));
			...
			for (int i = 0; i < numrows; i++) {
				ItemDescription_Controller newResult = makebasicsearch1(jsonResults
						.getJSONObject(i));
				bs2.add(newResult);
			}
		} catch (Exception e) {
