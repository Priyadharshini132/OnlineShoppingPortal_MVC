...
public class AdvancedSearch_Model {
	...
	public ArrayList<AdvancedSearch_Controller> advance(String[] tokens,
			...
			if (tokens[0] != "women" && tokens[0] != "kid") {
				try {
					if (sortby != "Price ASC" && sortby != "Brand") {
						que7 = "select SD.* from shoe_det SD, brand B, cat_main CM, cat_attr CA, categories C, sub_categories SC, color CO, material MA, heel_lenght HL, size SZ where B.B_NAME LIKE '%"
								+ price1
								+ "%' AND SC.SC_NAME LIKE '%"
								+ price1
								+ "%' AND CO.COLOR LIKE '%"
								+ price1
								+ "%' AND  MA.MATERIAL LIKE '%"
								+ price1
								+ "%' AND SZ.size=6.5 AND C.CID=1 AND B.BID= CM.bid AND SC.SCID= CM.scid AND CM.shid= SD.SHID AND B.BID= CM.bid AND C.CID= CM.cid AND SZ.SZID=CA.szid AND SC.SCID= CM.scid AND CO.CLID= CA.clid AND  MA.MTID= CA.mtidAND HL.HLID= CA.hlid AND CA.shid= CM.shid AND CM.shid= SD.SHID AND SD.PRICE>'%"
								+ price1
								+ "%' AND SD.PRICE <'%"
								+ price2
								+ "%' GROUP BY (SD.SHID) order by  SD.PRICE desc;";
						...
						que7 = "select SD.* from shoe_det SD, brand B, cat_main CM, cat_attr CA, categories C, sub_categories SC, color CO, material MA, heel_lenght HL, size SZ where B.B_NAME LIKE '%"
								...
								+ "%' AND SD.PRICE <'%"
								+ price2
								+ "%' GROUP BY (SD.SHID) order by  SD.PRICE asc;";
						...
						que7 = "select SD.* from shoe_det SD, brand B, cat_main CM, cat_attr CA, categories C, sub_categories SC, color CO, material MA, heel_lenght HL, size SZ where B.B_NAME LIKE '%"
								...
								+ "%' AND SD.PRICE <'%"
								+ price2
								+ "%' GROUP BY (SD.SHID) order by  B.B_NAME asc;";
					}
					...
					for (int i = 0; i < rows; i++) {
						...
						AdvancedSearch_Controller newResult = makebasicsearch(jsonResults
								.getJSONObject(i));
						bs1.add(newResult);
					}
				} catch (Exception e) {
	...
	public static AdvancedSearch_Controller makebasicsearch(
