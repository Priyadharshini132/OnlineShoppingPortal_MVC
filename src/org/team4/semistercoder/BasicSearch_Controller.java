...
public class BasicSearch_Controller extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out
				.println("Object created for basic search model in order to query and get resultset");

		try {

			System.out
					.println("User input as an array is passed along with its length");
			result = searchservice.basicSearch(word);// calling
			System.out
					.println("Processing is done in basic search model and the resultset arraylist is saved in the result");
			System.out
			...
			if (len > 0) {
				System.out
						.println("Redirect to basic search view if result length > 0 ");
