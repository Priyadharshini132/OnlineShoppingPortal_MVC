...
public class AdvancedSearch_Model1 {
	...
	public ArrayList<AdvancedSearch_Controller1> advance(String word1)
		...
		try {
			...
			que8 = "select SD.SHID, B.B_NAME, SC.SC_NAME, C.C_NAME, MA.MATERIAL,HL.HEIGHT, SZ.SIZE, CO.COLOR "
					...
					+ "color CO, material MA, heel_lenght HL, size SZ where C.CID='"
					+ cid
					+ "' AND B.BID= CM.bid AND SC.SCID= CM.scid "
			...
			for (int i = 0; i < rows; i++) {
				...
				AdvancedSearch_Controller1 newResult = makebasicsearch(jsonResults
						.getJSONObject(i));
				bs1.add(newResult);
			}
	...
	public static AdvancedSearch_Controller1 makebasicsearch(
