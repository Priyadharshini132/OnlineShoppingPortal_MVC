...
public class AdvancedSearch_Model2 implements java.io.Serializable {
	...
	public ArrayList<AdvancedSearch_Controller2> advance(String word,
			String pricecon, String sortby) throws SQLException// this one is
																// called
	{
		...
		/*
		 * String phrase = word; String delims = "[ ]+"; tokens =
		 * phrase.split(delims);
		...
		try {
			...
			for (int i = 1; i < cat.length; i++) {
				...
				que3 = "SELECT SH.SHID, SH.PRICE, B.B_NAME, CO.COLOR,SH.DESCP,SH.SHOE_NAME, CTR.img "
						...
						+ "C.C_NAME like '%"
						+ cat[0]
						+ "%' and C.CID= CM.cid AND "
						+ "CM.shid= SH.SHID AND SH.DESCP LIKE '%"
						+ cat[i]
						+ "%' AND "
						+ "SH.PRICE >"
						+ min
				...
				while (rs1.next()) {
					...
					st.execute(que4);
					que5 = "update shoe_det set FLAG=(FLAG+1) WHERE SHID='"
							+ shid[k] + "';";
			...
			try {
				...
				que7 = "SELECT SH.SHID, SH.PRICE, B.B_NAME, CO.COLOR,SH.DESCP,SH.SHOE_NAME, "
						...
						+ "ORDER BY SH.FLAG DESC, B.B_NAME "
						+ sortbrand[1]
						+ ", "
						+ "SH.PRICE "
						+ sortprice[1]
				...
				System.out
						.println("Total Number of records retrieved from database: ");
				...
				for (int i = 0; i < rows; i++) {
					AdvancedSearch_Controller2 newResult = makebasicsearch(jsonResults
							.getJSONObject(i));
					bs1.add(newResult);
				}
			...
			/*
			 * rs1.close(); rs3.close(); conn.close(); st.close();
			 */
			st1.close();
		}
		return bs1;
	...
	public static AdvancedSearch_Controller2 makebasicsearch(
