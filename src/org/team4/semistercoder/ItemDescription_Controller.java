...
public class ItemDescription_Controller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		System.out
				.println("*****Inside Item description Controller Page!!******");

		String shid10 = request.getParameter("keyword");
		System.out
				.println("Since this page is triggered only on selecting one item, that particular item's ID is retrieved in form of string");
		int shid7 = Integer.parseInt(shid10);

		System.out
				.println("Type cast is done and ID is converted into datatype INT:");
		System.out.println(shid10);
		System.out
				.println("Object created for item description model in order to query and get resultset");
		...
		try {
			...
			System.out
					.println("Processing is done in Item description model and the resultset arraylist is saved in the result");
			System.out
		...
		if (len1 == 1) {
			System.out
					.println("Redirect to Item description view if result length = 1 since it should be exactly 1. ");
			response.sendRedirect("itedesp.jsp");
		} else {
			System.out
					.println("Redirect to result not found view if result length < 0");
