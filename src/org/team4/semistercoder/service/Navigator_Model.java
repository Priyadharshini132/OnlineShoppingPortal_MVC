...
public class Navigator_Model {
	...
	public ArrayList<Navigator_Controller> navi(String[] tokens)
			throws SQLException {
		...
		try {
			...
			if (tlen > 1) {
				que1 = "select s.SHID, s.PRICE, s.SHOE_NAME, ca.img, s.DESCP from categories cat, sub_categories sc, shoe_det s, cat_attr ca, cat_main cm where cat.C_NAME like '"
						+ tokens[0]
						+ "%' and sc.SC_NAME like '%"
						+ tokens[1]
						+ "%' and sc.SCID= cm.scid and cat.CID= cm.cid and cm.shid= s.SHID and ca.shid= cm.shid;";
			...
			for (int i = 0; i < rows; i++) {
				...
				Navigator_Controller newResult = makebasicsearch(jsonResults
						.getJSONObject(i));
				bs1.add(newResult);
			}
		} catch (Exception e) {
	...
	public static Navigator_Controller makebasicsearch(
