...
public class Navigator_Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		System.out
				.println("Object created for Navigator model in order to query and get the resultset");

		try {
			System.out.println("String array is passed to navigator model");
			result = nmodel.navi(tokens);
			System.out
					.println("Processing is done in navigator model and the resultset arraylist is saved in the result");
			System.out
			...
			if (rlen > 0) {
				System.out
						.println("Redirect to navigator view if result length > 0. In navigotor, result len is always more than one.");
